...
from numpy import vectorize # Add the sleep () function -> To put the program in pause a few seconds
...
for face_color, axis in faces.values():
	vec_axis = vec(axis[0],axis[1],axis[2])
	for x in (-1, 0, 1):
		for y in (-1, 0, 1):
			...
			cos_angle = dot(vec(0, 0, 1), vec_axis)
			pivot = (cross(vec(0, 0, 1), vec_axis) if cos_angle == 0 else vec(1, 0, 0))
	...
	if key[0] in faces:
		...
		for r in arange(0, angle, angle / fps):
			...
			for sticker in stickers:
				if dot(sticker.pos, vec_axis) > 0.5:
					sticker.rotate(angle=angle / fps, axis=vec_axis,
								   origin=vec(0, 0, 0))
	elif key[0] == 'E':
		axis = vec(0, 0.5, 0)
		angle = ((pi / 2) if len(key)>1 else -pi / 2)
		for r in arange(0, angle, angle / fps):
			rate(fps)
			for sticker in stickers:
				sticker.rotate(angle=angle / fps, axis=axis,origin=vec(0, 0, 0))
...
def melanger(nbrDeMoves=25, show=1):
	...
	for i in range(nbrDeMoves):
		aleatoire = int(random.randint(0,11))
		#aleatoire = 5
...
def solveFinal(show=1):
	...
	while 1: # Boucle
		...
		if 'g' in ubr.values() and 'o' in ubr.values():
			move("U", show)
			move("U", show)
			move("R", show)
			move("U'", show)
